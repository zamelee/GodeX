...
**触发方式：只有一种 —— 用户手动点 Studio 里的 [探测模型] 按钮。**
**不做任何自动触发。配置改了、模型切了、Provider 换了，缓存的探测结果都不动。**
什么时候结果"过时"了，由用户自己判断要不要重跑。

- **`-U` 的语义**：探测时刻的那个 (模型, Provider) 组合下，这个 cap 被上游拒了。**只对那一次的组合有效**。
- 缓存何时"过时"（用户自己判断，不是系统判）：
  - 切模型（同一 Provider 下从 M2.7 切到 M2.7hs / M3）：旧结果对这个新模型没意义
  - 切 Provider / 上游网关：`-U` 是旧 gateway 的限制，新 gateway 可能从 `-U` 变 `+`
- 缓存仍然有效的场景（不用重跑）：
  - 只换 API key（同 Provider 同模型）
  - 只调温度 / top_p / max_tokens / 余量百分比等调用参数
- 探测程序永远跑全部 fixture，不跳过任何 -U-prone 的 cap（避免历史 -U 把 cap 永久排除）。
- Studio 不弹"配置变了，建议重测"的提示（按用户要求不加）。
